  ...
  return (
    <>
      ...
      <motion.div
        ...
        variants={fadeIn}
        viewport={{ once: true }}
        className="bg-gradient-to-br from-[#066E63] to-[#0A8A7B] text-white rounded-2xl p-8 shadow-2xl max-w-6xl mx-auto my-12 relative overflow-hidden"
      >
        {/* Decorative elements */}
        ...
        >
          ...
          <div>
            <motion.h2
              variants={item}
              className="text-3xl font-bold mb-6 text-[#F9F871] text-center md:text-left"
            >
          ...
          >
            <Image
              src="/logo/s5.webp" // pump installation image
        ...
        <motion.div
          variants={item}
          className="bg-[#0A5E55]/90 rounded-xl p-6 my-8"
        >
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="space-y-6 pl-10">
              {[
                { step: "Site Assessment", desc: "Evaluate water source, usage requirements, and power availability" },
                { step: "System Design", desc: "Calculate head pressure, flow rate, and recommend optimal pump" },
                { step: "Quality Check", desc: "Inspect all components before installation" },
                { step: "Professional Installation", desc: "Certified technicians handle the complete setup" },
                { step: "Testing & Calibration", desc: "Verify performance and adjust settings" },
                { step: "User Training", desc: "Demonstrate operation and maintenance" }
              ].map((step, index) => (
                <div key={index} className="relative">
                  ...
                  <h4 className="font-semibold text-[#F9F871]">{step.step}</h4>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { title: "Energy Efficiency", desc: "Properly sized pumps save 20-50% on electricity" },
              { title: "Extended Equipment Life", desc: "Correct installation prevents premature wear" },
              { title: "Warranty Protection", desc: "Maintain manufacturer warranties with professional install" },
              { title: "Optimal Performance", desc: "Get maximum flow rate and pressure" },
              { title: "Safety Compliance", desc: "Meets all electrical and plumbing codes" },
              { title: "24/7 Support", desc: "Emergency service available" }
            ].map((benefit, index) => (
              <div key={index} className="bg-[#0A6E63] p-4 rounded-lg flex items-start">
                <svg className="w-5 h-5 text-[#F9F871] mr-3 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
          <Link
            href="/contact"
            className="bg-gradient-to-r from-[#F472B6] to-[#F9A8D4] text-white px-8 py-3 rounded-full text-lg font-semibold shadow-lg hover:shadow-xl transition-all inline-flex items-center"
          >
            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
            Get Your Free Pump Consultation
          </Link>
        </motion.div>
      </motion.div>
      <Footer />
